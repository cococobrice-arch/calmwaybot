...
Я приготовил материал, который поможет Вам разобраться, что запускает панические атаки, 
чем они поддерживаются и как наконец вернуться к расслабленной жизни.  
...
async def send_case_story(chat_id: int):
    await asyncio.sleep(5)
    text = (
        "Чтобы ослабить власть тревоги над нами, нам нужно начать делать то, что страшно.\n"
        "И я хочу показать, как это выглядит на практике.\n\n"
...
async def send_final_message(chat_id: int):
    ...
    text = (
        "Паническое расстройство почти никогда не возникает само по себе — "
        "за ним почти всегда стоят накопленное напряжение, сложности во взаимодействии с другими людьми, "
        "страх не справиться с жизнью и усталость от постоянной гиперответственности.\n\n"
        "С людьми, переживающими панические атаки, я работаю каждый день. "
        "Поэтому я хорошо знаю, как важно не откладывать обращение за помощью, "
        "потому что со временем тревога перестаёт быть лишь реакцией на стресс и начинает определять образ жизни.\n\n"
        "На индивидуальных консультациях мы можем вместе разобраться, из чего складывается именно Ваш цикл тревоги: "
        ...
        'то подробнее о том, как я могу Вам помочь, Вы можете почитать по <a href="https://лечение-паники.рф/что-делает-психиатр">ссылке</a>.'
    )
    await bot.send_message(chat_id, text, parse_mode="HTML", disable_web_page_preview=True)
    upsert_user(chat_id, step="final_message_sent")
    log_event(chat_id, "bot_final_message_sent", "Отправлено финальное сообщение")
