...
async def send_case_story(chat_id: int):
    ...
    text = (
     ...
     "Тревога заставляет нас избегать пугающих ситуаций и тем самым только укрепляет свою власть."
     "Теперь я хочу показать, как это выглядит на практике — в реальной истории из моего гайда. \n\n"   
    ...
    "<u>выполнять дела не на «5», а на «4»</u> и не проверять каждое своё слово — внутреннее напряжение стало спадать. "
...
async def send_chat_invite(chat_id: int):
    await asyncio.sleep(5)
    text = (
        "Истории как у этой девушки откликаются многим людям. Почти у каждого, кто сталкивается с паникой, есть похожие страхи, сомнения и попытки справиться в одиночку.\n"
        "Но путь восстановления становится легче, когда Вы понимаете, что не только Вам приходится через это проходить.\n\n"
